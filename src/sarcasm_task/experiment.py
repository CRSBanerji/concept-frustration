...
def run_one_fold(
    *,
    ...
    # Fair comparison on shared concept pair (C1, C2)
    W2_12 = W2[:2, :]
...
def save_results(rows, out_path=None):
    df = pd.DataFrame(rows)
    if out_path is None:
        out_path = RESULTS_CSV
    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    df.to_csv(out_path, index=False)
    return df, out_path


def summarize_results(rows_or_df):
...
def main():
    ...
    rows = run_experiment()
    df, out_path = save_results(rows, RESULTS_CSV)
    print("\nSaved results:", out_path)
    print(summarize_results(df)[["mean", "std", "min", "max"]])
